...
const checkDidNotRateUser = (req,res,next) => {
...
//check if req.user did not rate res.eventId_event
const checkDidNotRateEvent = (req,res,next) => {
  if(res.eventId_event.wasRatedBy(req.user)){
    let err = new Error('User is not authurized for this action.');
    err.status = 401;
    err.name = 'Permission Denied';
    err.target = 'user';
